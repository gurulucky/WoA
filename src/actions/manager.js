...
export const getNFTsWithHighResImage = async (nftIds) => {
  ...
  for (let i = 0; i < tokenURIs.length; i++) {
    try {
      ...
      nfts.push({ ...nft, image: nft.image.replace('https://ipfs.infura.io', 'https://gateway.pinata.cloud'), metadataUri: tokenURIs[i], tokenId: nftIds[i], highUri: highURIs[i], rarityScore: rarityScores[i] })
    } catch (err) {
